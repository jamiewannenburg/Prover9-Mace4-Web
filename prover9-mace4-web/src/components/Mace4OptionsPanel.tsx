...
const Mace4OptionsPanel: React.FC = () => {
  const { options, setOptions } = useMace4Options();

  const handleIntegerParameterChange = (name: string, value: number) => {
    setOptions({
      [name]: {
        ...(options[name as keyof Mace4Options] as IntegerParameter),
        value
      }
    });
  };
  ...
  const handleStringParameterChange = (name: string, value: string) => {
    setOptions({
      [name]: {
        ...(options[name as keyof Mace4Options] as StringParameter),
        value
      }
    });
  };
  ...
  const handleFlagChange = (name: string, checked: boolean) => {
    setOptions({
      [name]: {
        ...(options[name as keyof Mace4Options] as Flag),
        value: checked
      }
    });
  };
